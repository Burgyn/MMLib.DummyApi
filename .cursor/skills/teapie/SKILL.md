...
- **` -req.http`** (required) - HTTP request file (one or more requests)
- **` -init.csx`** (optional) - Pre-request script for setup
- **` -test.csx`** (optional) - Post-response script for validation
...
- `## AUTH-PROVIDER: ` - Set authentication provider
- `## RETRY-STRATEGY: ` - Select retry strategy
- `## RETRY-UNTIL-TEST-PASS: ` - Retry until all test assertions pass
...
- **For JSON arrays**: Use string checks (simple) or `System.Text.Json.JsonSerializer.Deserialize ()` (complex)
...
**Arguments:**
- ` ` - Name of test case (required). Can include numeric prefix: `001-Create-Car`
...
**Options:**
- `-i`, `--init`, `--pre-request` - Generate pre-request script (` -init.csx`)
- `-t`, `--test`, `--post-response` - Generate post-response script (` -test.csx`)
